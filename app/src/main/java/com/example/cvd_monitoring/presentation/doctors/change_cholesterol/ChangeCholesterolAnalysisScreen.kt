...
import com.example.cvd_monitoring.presentation.doctors.change_blood.ChangeBloodAnalysisViewModel
import kotlinx.coroutines.flow.collectLatest
@RequiresApi(Build.VERSION_CODES.O)
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun ChangeCholesterolAnalysisScreen(
    navController: NavController,
    slug: String,
    viewModel: ChangeCholesterolAnalysisViewModel = hiltViewModel(),
) {

    LaunchedEffect(key1 = slug) {
        viewModel.getCholesterolAnalysis(slug)
    }

    val cholesterol = viewModel.cholesterolState.value
    val triglycerides = viewModel.triglyceridesState.value
    val hdlCholesterol = viewModel.hdlCholesterolState.value
    val ldlCholesterol = viewModel.ldlCholesterolState.value
    ...
    Column(
        ...
        Button(
            onClick = {
                viewModel.updateCholesterolAnalysis()
